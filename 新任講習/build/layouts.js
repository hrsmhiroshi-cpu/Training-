...
function rows(pres, d, ctx) {
  ...
  items.forEach((it, i) => {
    ...
    s.addText([
      ...
      en(it.en, { fontSize: it.small ? 15 : 16 }),
...
function quote(pres, d, ctx) {
  ...
  s.addText(d.quoteEn, {
    ...
    fontFace: F.en, fontSize: d.quoteEnSize || 16, color: 'DCEFEB', lineSpacingMultiple: 1.0,
  });
  if (d.source) {
    s.addText(d.source, {
  ...
  if (d.note) {
    ...
    s.addText([
      ...
      en(d.noteEn, { fontSize: 15, color: C.ink }),
...
function ttx(pres, d, ctx) {
  ...
  s.addText([
    ...
    en(d.scenarioEn, { fontSize: 14.5, color: 'DCEFEB' }),
...
function topics(pres, d, ctx) {
  ...
  list.forEach((t, i) => {
    ...
    s.addText([
      ...
      en(t.hEn, { fontSize: 15, color: C.teal }),
    ...
    t.b.forEach((bl, k) => {
      ...
      runs.push(en(bl.en, { fontSize: 14, breakLine: !last }));
    });
    s.addText(runs, {
...
function debrief(pres, d, ctx) {
  ...
  const ans = d.answers;
  const gap = 0.11;
  // Answers differ a lot in length. Equal rows make the long ones spill over the
  // short ones, so the band is shared out in proportion to the wrapped line count.
  const tw = G.CW - 0.96;
  const jaLines = (t, fs) => Math.max(1, Math.ceil(t.length / Math.floor(tw / (fs / 72))));
  const enLines = (t, fs) => Math.max(1, Math.ceil(t.length / Math.floor(tw / (fs * 0.47 / 72))));
  const needs = ans.map((a) =>
    0.145 * jaLines((d.qLabel || '問') + ((d.startN || 1)) + '　' + a.q, 11.5)
    + 0.19 * jaLines(a.a, 15) + 0.19 * enLines(a.aEn, 15) + 0.16);
  const needSum = needs.reduce((x, y) => x + y, 0);
  const band = bot - top - gap * (ans.length - 1);
  const heights = needs.map((n) => band * n / needSum);
  ...
  ans.forEach((a, i) => {
    ...
    s.addText([
      ...
      en(a.aEn, { fontSize: 15 }),
    ], { x: G.M + 0.70, y: y + 0.07, w: tw, h: rh - 0.14, valign: 'middle', margin: 0, lineSpacingMultiple: 0.88 });
  });
  ...
  if (d.split) {
    ...
    s.addText([
      ...
      en(d.splitEn, { fontSize: 13, color: 'DCEFEB' }),
